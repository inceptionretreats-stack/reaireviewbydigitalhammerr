 ...
 *
 *   node --env-file=.env scripts/ops/create-admin.mjs --email you@example.com --password 'a strong one'
 *   node --env-file=.env scripts/ops/create-admin.mjs --email you@example.com --password '...' --name "Your Name"
 *
...
const reason =
  opt('reason')?.trim() || 'Platform admin account created with scripts/ops/create-admin.mjs';
const totpSecret = opt('totp-secret')?.trim();

if (!email || !password) {
  console.error(
    'Usage: node --env-file=.env scripts/ops/create-admin.mjs --email <email> --password <password> [--name <name>]',
  );
  process.exit(1);
}
if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
