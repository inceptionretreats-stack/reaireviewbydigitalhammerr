 ...
 *
 * `components/onboarding/AiContextStep.tsx` carries a near-identical private copy of this and says
 * in its own comments that the parsing wants a shared home in `lib/` the moment a second screen
 * needs a non-POST call. That moment is this module; `lib/` was outside this change, so the
 * duplicate is raised as a concern rather than left silent.
 *
