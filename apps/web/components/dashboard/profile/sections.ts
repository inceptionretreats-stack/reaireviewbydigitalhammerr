 ...
 *
 * It lives under `components/` and is imported by `app/api/v1/business/links/**` because those two
 * route files are the only API paths this module owns — there is nowhere shared to put it yet. It
 * carries no React import, no `'use client'` and nothing server-only, so it is safe in a route
 * handler and in a browser bundle alike. See concerns: `apps/web/lib/` is where it belongs.
 */
 ...
 * `app/[slug]/page.tsx` enforces that twice: disabled rows are excluded in SQL, and a row whose
...
export function rendersPublicly(
  ...
  // contradicts the page it mirrors: `resolveTarget` in `app/[slug]/page.tsx` resolves it as
