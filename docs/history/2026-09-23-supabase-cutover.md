# Supabase cutover — 23 September 2026

> **Historical record.** How the production PostgreSQL database moved from Neon to the Digital
> Hammerr Supabase project on 23 September 2026, and the additive migration applied on 24
> September. This text was moved out of `docs/operations/database-supabase.md` on 24 September 2026
> with its substance unchanged. Counts, deployment IDs, hashes and paths are as they were on those
> dates, not current values. To operate the database now, use
> [database on Supabase](../operations/database-supabase.md).
>
> **Paths outside the repository.** `tmp/` was a git-ignored folder on the owner's machine and was
> never part of the repository. The one-off cutover scripts that were in `tmp/supabase/` are now
> kept in the owner's private backup folder,
> `ai-review-private-backups/2026-09-23-supabase/tooling/`, and the portable PostgreSQL client
> that was in `tmp/tools/postgresql-18.6-4-portable/` is now in
> `ai-review-private-backups/tools/postgresql-18.6-4-portable/`. The scripts import
> `scripts/supabase/*.mjs` by relative path, so they no longer run from there. Do not rerun them.
>
> The application records counted below were removed later the same day by the owner-requested
> [fresh-start reset](2026-09-23-fresh-start-reset.md). Do not restore them.

## Additive migration 0008 — 24 September 2026
...
verification remains a separate paused task. The Vercel account name does not determine Supabase
ownership.
...
The final access-restricted backup is outside the repository, in the owner's private backup folder:

`ai-review-private-backups/2026-09-23-supabase/2026-09-23T09-51-12-621Z-ed082ab3`
...
- Before promotion, ten candidate HTTP checks passed: homepage/signup/login returned 200,
  invalid signup returned 422, an unknown QR returned 404, and all five saved QR routes
  returned 200. All ten checks passed again on the public site after promotion. The full
  offline suite passed **1,526 tests across 93 files**; the migration guard checked all eight
  migrations with the known manually managed analytics-partition modeling warning.
...
### Source observations before the cutover
...
### Restore rehearsal and private backup
...
is recorded in `tmp/tools/postgresql-18.6-4-portable/PROVENANCE.md` (since moved, with the client,
to `ai-review-private-backups/tools/postgresql-18.6-4-portable/PROVENANCE.md`).

The earlier rehearsal backup is outside the repository, in the owner's private backup folder;
**it is not the final cutover backup**:

`ai-review-private-backups/2026-09-23-supabase/2026-09-23T09-06-57-578Z-561bbea4`
...
Operational helpers are in ignored `tmp/supabase/` (since moved to
`ai-review-private-backups/2026-09-23-supabase/tooling/`); never print restored SQL or secret env
files. **Do not rerun `restore.mjs` against the live target.** The source was still writable at
this earlier rehearsal checkpoint, so that copy was not promoted. The fresh fenced backup above
...
At the time of the cutover the repository contained **eight migrations (`0000`–`0007`) and 34
declared application tables**, including `maintenance_jobs`. Monthly analytics partitions are
additional physical tables, not separate Drizzle schema declarations. The permissions script
covers the declared tables and discovers analytics partitions dynamically; its unit test compares
the allowlist with the current schema exports so new application tables cannot silently be
omitted.

## Setup procedure used for the cutover — not instructions to reprovision the live target
...
## Migration procedure used for the cutover — do not rerun against the live target
